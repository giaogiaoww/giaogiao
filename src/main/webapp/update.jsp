<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"  import="com.wyj.pojo.*"%>
		...
		<DIV class=dialog>
			...
			<DIV class=box>
				<FORM id=add_action method=post name=add
					...
					<input type="hidden" value="${details.hid}" name="hid" /> 
					<DIV class=infos>
						<TABLE class=field>
							<TBODY>
								<TR>
									...
									<TD><INPUT id=add_action_title class=text type=text
										name=title value="${details.title}"></TD>
								</TR>
								<TR>
									<TD class=field>户 型：</TD>
									<TD><SELECT class=text name=type_id><OPTION value=1
												${details.typeid==1?"selected":""}>一室一厅</OPTION>
											<OPTION value=2 ${details.typeid==2?"selected":""}>一室两厅</OPTION>
											<OPTION value=3 ${details.typeid==3?"selected":""}>两室一厅</OPTION>
											<OPTION value=4 ${details.typeid==4?"selected":""}>两室两厅</OPTION></SELECT></TD>
								</TR>
								<TR>
									<TD class=field>面 积：</TD>
									<TD><INPUT id=add_action_floorage class=text type=text
										value="${details.area}" name=floorage oninput="value=value.replace(/[^\d]/g,'')"></TD>
								</TR>
								<TR>
									<TD class=field>价 格：</TD>
									<TD><INPUT id=add_action_price class=text type=text
										value="${details.price}" name=price oninput="value=value.replace(/[^\d]/g,'')"></TD>
								</TR>
								<TR>
									<TD class=field>房产证日期：</TD>
									<TD><INPUT readonly="readonly" style="width: 250px;" value="${details.housedate.getYear()+1900}-${details.housedate.getMonth()+1}-${details.housedate.getDate()}"
								...
								<TR>
									...
									<TD>区：<SELECT class=text name=district_id>
											<c:forEach items="${map.keySet()}" var="ii" varStatus="index">
												<OPTION ${details.address.substring(0,3)==ii?'selected':''}>${ii}</OPTION>
											</c:forEach>
								...
								<TR>
									...
									<TD><INPUT id=add_action_contact class=text type=text value="${details.phone}"
								...
								<TR>
									...
									<TD><TEXTAREA name=description>${details.detailed}</TEXTAREA></TD>
								</TR>
							</TBODY>
						</TABLE>
						<DIV class=buttons>
	...
	$(function() {
		...
		function ddd() {
			$.post("ajax111", {
				...
				for ( var ii in rs) {
					var aa="";
					if(rs[ii]== "<%=request.getAttribute("details")==null?"":((House)(request.getAttribute("details"))).getAddress().substring(3, ((House)(request.getAttribute("details"))).getAddress().length())%>" ){
						aa="selected";
					}
					$("[name=street_id]").append("<OPTION "+aa+">" + rs[ii] + "</OPTION>");
